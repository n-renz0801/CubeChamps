...
def sort_key(s):
    """Universal sort key: more attempts first, then lower average wins."""
...
def create_meet():
    if request.method == 'POST':
        ...
        for name, rcount in zip(event_names, rounds_list):
            event = Event(name=name, cubemeet_id=new_meet.id)
            db.session.add(event)
            db.session.commit()

            for i in range(int(rcount)):
                db.session.add(Round(event_id=event.id, round_number=i + 1))
...
def meet_detail(meet_id):
    meet = CubeMeet.query.get_or_404(meet_id)
    return render_template("meet_detail.html", meet=meet, not_homepage=True)


@app.route("/event/<int:event_id>/add_round", methods=["POST"])
def add_round(event_id):
    event     = Event.query.get_or_404(event_id)
    new_round = Round(event_id=event.id, round_number=len(event.round_list) + 1)
...
def add_event(meet_id):
    ...
    new_event = Event(name=event_name, cubemeet_id=meet_id)
    db.session.add(new_event)
    db.session.commit()

    for i in range(rounds_count):
        db.session.add(Round(event_id=new_event.id, round_number=i + 1))
...
def round_detail(round_id):
    ...
    meet      = event.cubemeet
    solves    = sorted(Solve.query.filter_by(round_id=round_id).all(), key=sort_key)
...
def add_solver(event_id, round_number):
    ...
    competitor = Competitor(
        name=f"__tmp_{uuid.uuid4().hex[:8]}",
...
def update_solve(solve_id):
    ...
    if name == "":
        ...
        db.session.commit()
        return jsonify({"deleted": True, "solve_id": solve_id})
...
def persons(meet_id):
    ...
    return render_template("persons.html", competitors=competitors, meet=meet, not_homepage=True)
...
def person_detail(meet_id, competitor_id):
    ...
    for event_id, data in events_map.items():
        ...
        for solve in sorted(data['rounds'], key=lambda s: s.round.round_number):
            ...
            sorted_round     = sorted(all_round_solves, key=sort_key)
...
def global_person_detail(competitor_name):
    ...
    events_map = {}
    for solve in solves:
        event_name = solve.round.event.name
        times      = [solve.attempt1, solve.attempt2, solve.attempt3, solve.attempt4, solve.attempt5]
        ...
        if single is not None:
            current = events_map[event_name]['best_single']
            if current is None or single < current:
                events_map[event_name]['best_single'] = single

        if avg is not None and avg != 'DNF':
            current = events_map[event_name]['best_avg']
            if current is None or avg < current:
...
def podiums(meet_id):
    ...
    for event in meet.events:
        ...
        # Fetch all solves then filter out temp/empty competitors in Python
        all_solves = Solve.query.filter_by(round_id=last_round.id).all()
        solves = [
            s for s in all_solves
            if s.competitor.name
            and s.competitor.name.strip() != ''
            and not s.competitor.name.startswith('__tmp_')
        ]
